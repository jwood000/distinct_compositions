...
\begin{definition}
    The set of all distinct $k$-compositions of $n$ is given by:
    \[
        \mathcal{D}(n, k) =
            \left\{
                \gamma = (\gamma_1, \dots, \gamma_k)
                \mid
                \gamma_i \in \mathbb{N},
                \gamma_i \neq \gamma_j \text{ for } 1 \leq i < j \leq k,
                \quad \sum_{i = 1}^{k} \gamma_i = n
            \right\}
    \]
\end{definition}
...
\begin{definition}
    The set of all distinct compositions of $n$ is given by:
    \[
        \mathcal{D}(n) = \bigcup_{k = 1}^{n} \mathcal{D}(n, k)
    \]
\end{definition}

\begin{definition}
    Let $\gamma = (\gamma_1, \dots, \gamma_k)$ and $\gamma^{\prime} = (\gamma^{\prime}_1, \dots, \gamma^{\prime}_k)$ be two different $k$-compositions of $n$ and let $i$ be the first index from the left where they differ.
    \[
